...
Die wichtigsten Ergebnisse sind konservativ, aber klar. Die Reproduktion bestätigt die Richtung von Meyer et al.: Bei Trainingsgröße 600 erreicht das unbeschränkte Edge-Modell $0.627\pm0.013$ Test Accuracy, während das $D_4$-äquivariante Edge-Modell $0.687\pm0.020$ erreicht. Der Generalization Gap sinkt von $0.070$ auf $0.021$. Die partielle Symmetrie $C_4$ erreicht fast dieselbe Leistung wie $D_4$ und liegt in einzelnen Regimen leicht vorn, aber nicht robust genug für den Claim, dass weniger Symmetrie grundsätzlich besser sei. Der stärkste Effekt entsteht durch die winning-line gates: Das beste $D_4$-äquivariante Edge-plus-Line-Modell erreicht bei 600 Trainingsbeispielen $0.785\pm0.021$ Test Accuracy, also deutlich mehr als der Meyer-artige Edge-Ansatz mit $0.687\pm0.020$.
...
\Cref{fig:protocol} fasst den Designraum zusammen. Panel (a) zeigt die $D_4$-Symmetrie des Spielfelds. Panel (b) zeigt die Datenkodierung und die durch Untergruppen induzierten Orbits. Panel (c) zeigt den Meyer-artigen äquivarianten Gateset mit orbit-geteilten Single-Qubit-Gates und $CR_P$-Paaren. Panel (d) zeigt unsere winning-line-Erweiterung. Wichtig ist: Die Erweiterung ist kein symmetry breaking. Die zusätzlichen Gates werden ebenfalls über Gruppenorbits geteilt und bleiben daher äquivariant.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{images/fig_protocol.pdf}
\caption{Designraum der Arbeit. Tic-Tac-Toe-Labels sind invariant unter den acht Rotationen und Spiegelungen des Bretts. Die Brettwerte werden auf neun Qubits kodiert, und Untergruppen erzeugen unterschiedliche Parameter-Sharing-Orbits. Der Meyer-artige Edge-Ansatz nutzt orbit-geteilte Single-Qubit-Gates und $CR_P$-Paare; die eigene Erweiterung ergänzt orbit-geteilte $ZZZ$- und $CCRZ$-Gates auf den acht Gewinnlinien, ohne die Äquivarianz zu brechen.}
\label{fig:protocol}
\end{figure}
...
\]
Trainings- und Testsets werden pro Seed balanciert gesampelt. Die finale Auswertung verwendet 600 Testbeispiele und die Trainingsgrößen $30,60,120,240,450,600$. Wo strikt disjunkte balancierte Splits bei den größten Trainingsgrößen nicht möglich sind, wird der im CSV dokumentierte Fallback-Split konsistent verwendet.
...
\Cref{fig:main} zeigt die zentrale Evidenz. Die Reproduktion bestätigt die erwartete Richtung. Das unbeschränkte Edge-Modell erreicht bei 600 Trainingsbeispielen $0.627\pm0.013$ Test Accuracy. Das $D_4$-äquivariante Edge-Modell erreicht $0.687\pm0.020$, und unsere edge+lines/$D_4$-Erweiterung steigt weiter auf $0.785\pm0.021$. Gleichzeitig sinkt der Generalization Gap von $0.070$ auf $0.021$ für den Meyer-artigen $D_4$-Vergleich. Das freie Modell besitzt mit 204 Parametern deutlich mehr Freiheitsgrade als das $D_4$-Modell mit 54 Parametern, aber diese zusätzlichen Freiheitsgrade helfen auf dem Testset nicht.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{images/fig_main_evidence.pdf}
\caption{Hauptergebnisse aus dem finalen 10-Seed-Protokoll. Links: 600er Endpunkt mit keiner Symmetrie, Meyer-artigem edge/$D_4$ und unserer edge+lines/$D_4$-Erweiterung. Mitte: Untergruppen-Sweep über Trainingsgrößen $30$ bis $600$; $C_4$ folgt $D_4$ sehr eng. Rechts: Edge-Ansatz und Edge-plus-Line-Erweiterung über dieselbe Trainingsgrößenachse; die Winning-Line-Gates verbessern die Accuracy deutlich.}
\label{fig:main}
\end{figure}
...
Der Untergruppen-Sweep zeigt ein differenzierteres Bild als ein reines none-vs-$D_4$-Experiment. In \Cref{tab:partial} sind die wichtigsten Werte zusammengefasst. $C_4$ erreicht über alle Datenregime hinweg fast dieselbe mittlere Test Accuracy wie $D_4$: $0.631$ gegenüber $0.632$. Bei 30 Trainingsbeispielen liegt $C_4$ mit $0.552$ über $D_4$ mit $0.532$; bei 120 Beispielen liegen beide nah beieinander; bei 600 Beispielen erreichen $C_4$ und $D_4$ $0.695$ und $0.687$.

\begin{table}[t]
\centering
\caption{Partielle Äquivarianz im Edge-Ansatz. Werte sind mittlere Test Accuracy über zehn Seeds.}
\label{tab:partial}
\begin{tabular}{lcccc}
\toprule
Gruppe & Parameter & Test@30 & Test@600 & Mean Test\\
\midrule
none & 204 & 0.444 & 0.627 & 0.537\\
$Z_2$ rot & 108 & 0.495 & 0.667 & 0.585\\
$Z_2$ refl & 126 & 0.481 & 0.671 & 0.578\\
$C_4$ & 60 & 0.552 & 0.695 & 0.631\\
$D_2$ & 78 & 0.550 & 0.678 & 0.611\\
$D_4$ & 54 & 0.532 & 0.687 & 0.632\\
\bottomrule
\end{tabular}
\end{table}

Panel (b) in \Cref{fig:main} zeigt diese Dynamik über die gesamte Trainingsgrößenachse. Die Interpretation sollte vorsichtig sein. Es gibt kleine Bereiche, in denen $C_4$ etwas besser ist. Das reicht nicht für die Aussage, dass $C_4$ allgemein besser sei. Es reicht aber für eine interessante praktische Aussage: Rotation-only Equivariance kann auf diesem Benchmark fast denselben Nutzen liefern wie volle $D_4$-Equivariance, obwohl Spiegelungsconstraints gelockert werden. Der Nutzen der vollen Gruppe ist also nicht als binäres Dogma zu verstehen.

\subsection{Random-Sharing-Control: Nicht nur weniger Parameter}\label{subsec:random}

Ein naheliegender Einwand lautet: Vielleicht hilft Symmetrie gar nicht als Struktur, sondern nur als Parameterreduktion. Dann wäre jedes zufällige Sharing mit derselben Parameterzahl ähnlich gut. Genau deshalb ist der Random-Sharing-Control wichtig. Panel (a) in \Cref{fig:controls} zeigt, dass dieser Einwand nicht ausreicht. Bei $D_4$ erreicht symmetry-aware sharing $0.687$, random sharing mit gleicher Parameterzahl aber nur $0.572$. Bei $C_4$ erreicht symmetry-aware sharing $0.695$, random sharing nur $0.561$.
...
\subsection{Task-Aligned Equivariant Gates: der stärkste Effekt}\label{subsec:linegate-results}

Der deutlichste empirische Gewinn entsteht durch die winning-line gates. Panel (c) in \Cref{fig:main} zeigt Edge und Edge-plus-Line über dieselbe Trainingsgrößenachse. Das Meyer-artige Edge/$D_4$-Modell erreicht bei 600 Trainingsbeispielen $0.687\pm0.020$ Test Accuracy. Das beste Edge-plus-Line-Modell, \texttt{edge\_line\_zzz\_ccrz}/$D_4$, erreicht $0.785\pm0.021$. Der $C_4$-Ableger derselben Architektur ist mit $0.771\pm0.019$ ebenfalls stark, liegt aber im finalen Endpunkt unter $D_4$.
...
\caption{Controls und Ablationen aus dem finalen 10-Seed-Protokoll. Links: Parameter-matched random sharing schlägt group-orbit sharing nicht. Mitte: Ablation der Line-Gate-Familien bei 600 Trainingsbeispielen, aufsteigend nach mittlerer Leistung, mit dem markierten originalen Edge/$D_4$-Ansatz als Referenz. Rechts: Accuracy ist nicht allein durch Parameterzahl erklärbar; task-aligned equivariant geometry verschiebt das Modell in einen anderen Leistungsbereich.}
\label{fig:controls}
\end{figure}
...
\subsection{Symmetrie als Designachse}\label{subsec:break}

Die Experimente zeigen Symmetrie als Designachse statt als binäre Entscheidung. Die Frage ist nicht nur, ob man volle $D_4$-Symmetrie erzwingen oder bewusst abschwächen sollte. $C_4$ ist kompetitiv und in kleinen Datenregimen punktuell besser, aber es schlägt $D_4$ nicht robust. Die saubere Schlussfolgerung ist daher nicht, dass man Symmetrie brechen muss, sondern dass der Symmetriegrad eine sinnvolle Modellwahlachse ist.

Die winning-line-Ergebnisse zeigen zugleich, dass der stärkere Hebel innerhalb des äquivarianten Designraums liegt. Man muss $D_4$ nicht brechen, um den Meyer-artigen $D_4$-Ansatz zu übertreffen; man kann bessere orbit-geteilte Gates auf die Motive setzen, die das Label bestimmen. Der finale Titel \emph{Symmetry as a Design Axis in Variational Quantum Learning} fasst genau diese konservativere Aussage zusammen.
...
Die Ergebnisse bleiben finite-seed Simulationen. Die wichtigsten Sweeps verwenden zehn Seeds und ein gemeinsames Trainingsprotokoll. Außerdem ist die Architektur Meyer-artig, aber keine Byte-für-Byte-Reproduktion des Originalcodes. Die Richtung des Meyer-Effekts wird reproduziert, die absoluten Zahlen sollten jedoch als Ergebnis unserer Implementierung verstanden werden.
...
\noindent\textcolor{red}{\Large Wortanzahl fachlicher Haupttext: \textbf{3363}}

\vspace{0.3cm}
\noindent\textcolor{red}{\Large Zeichenanzahl ohne Leerzeichen: \textbf{21350}}

\vspace{0.3cm}
\noindent\textcolor{red}{\Large Zeichenanzahl mit Leerzeichen: \textbf{24604}}

\vspace{0.5cm}
\noindent\emph{Hinweis:} Die Wortzählung bezieht sich auf den fachlichen Haupttext in \texttt{contents.tex} bis einschließlich Fazit und wurde mit \texttt{texcount} bestimmt. Die Zeichenzählung ist eine bereinigte Näherung ohne LaTeX-Kommandos, Literaturbefehle, Tabellenkörper und Gleichungen.
